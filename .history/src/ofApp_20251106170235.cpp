...
void ofApp::draw(){
    ...
    ofNoFill();
    ofBeginShape();
    for (int i = 0; i < fourierSpectrum.size(); i += 8) { // pula alguns pontos
        float x = ofMap(i, 0, fourierSpectrum.size(), 0, ofGetWidth());
        float y = ofGetHeight() - ofMap(fourierSpectrum[i], 0, 200, 0, ofGetHeight()/2, true);
        ofVertex(x, y);
    }
    ofEndShape();
}
...
void ofApp::computeFourier(const std::vector<float>& sinalwave) {
    int N = signal.size();  // number of samples
    ...
    for (int k = 0; k < N; k++) {
        ...
        for (int n = 0; n < N; n++) {
            ...
            F1[k] += sinalwave[n] * cos(phi);
            F2[k] -= sinalwave[n] * sin(phi);
        }

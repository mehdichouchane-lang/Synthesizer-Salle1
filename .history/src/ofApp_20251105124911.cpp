...
float xOffset = 50;              // deslocamento horizontal da borda esquerda
...
int blackKeyPattern[7] = {0,1,0,1,1,0,1}; // padrão real
float blackWidth = keyWidth * 0.6;
float blackHeight = keyHeight * 0.6;

for(int i = 0; i < nKeys; i++){
    int patternIndex = i % 7;

    // desenhar preta apenas se pattern=1 e não for a primeira branca
    if(blackKeyPattern[patternIndex] && i > 0){
        float bx = xOffset + i * keyWidth - blackWidth/2; // centralizar entre brancas
        float by = yOffset;

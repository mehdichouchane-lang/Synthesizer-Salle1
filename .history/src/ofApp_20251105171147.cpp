...
void ofApp::draw(){
	...
	ofDrawBitmapString("press 's' to unpause the audio\npress 'e' to pause the audio", 31, 80);
...
float keyWidth = 80;             // largura de cada tecla
float keyHeight = 220;           // altura de cada tecla
...
for(int i = 0; i < nKeys; i++){
    ...
    ofSetColor(255);
ofDrawBitmapString("Clavier: Q W E R T Y U I O P -> 10 notes (C4 à E5)", 32, 470);
ofDrawBitmapString("Note active: " + ofToString(targetFrequency, 2) + " Hz", 32, 490);
...
int octaveShift = 12; // 0 = normal, +12 = uma oitava acima, -12 = abaixo
float getFrequencyFromMidi(int midiNote) {
    return 440.0f * pow(2.0f, (midiNote + octaveS - 69) / 12.0f);
}
...
void ofApp::keyPressed  (int key){
	if (noteMapMidi.find(key) != noteMapMidi.end()) {

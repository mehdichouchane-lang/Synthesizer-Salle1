...
ofSetColor(255);
ofDrawBitmapString("KeyBoard: Q W E R T Y U I O P -> 10 notes (C4 to E5)", 32, 500);
...
float getFrequencyFromMidi(int midiNote) {
    return 440.0f * pow(2.0f, ( + octaveShift - 49) / 12.0f);
}

float getFrequencyFromKey(int pianoKeyNumber) {
    // pianoKeyNumber varia de 1 (A0) a 88 (C8)
    return 440.0f * pow(2.0f, (pianoKeyNumber - 49) / 12.0f);
}
...
std::map<int,int> noteMapMidi = {
    {'q',40}, {'w',42}, {'e',44}, {'r',45}, {'t',47},
    {'y',49}, {'u',51}, {'i',52}, {'o',54}, {'p',56}
};

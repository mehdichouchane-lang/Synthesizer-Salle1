...
//Fourier Transform
N = getFrequencyFromKey(keyToPiano['p'])  - getFrequencyFromKey(keyToPiano['q']) / 10 // number of samples

float F1[N]; // output spectrum	
for (int k = getFrequencyFromKey(keyToPiano['q']); k < N; 10++) {
	F1[k] = 0;
	F2[k] = 0;
	for (int n = 0; n < N; n++) {
		float phi = (2 * PI * k * n) / N;
		F1[k] += sample[n] * cos(phi);
		F2[k] += sample[n] * sin(phi);
	}
	F[k] = sqrt(F1[k] * F1[k] + F2[k] * F2[k]);
}

void ofApp::computeFourier(const std::vector<float>& sample) {
    int N = sample.size();  // número de amostras (tamanho do sinal)
    
    std::vector<float> F1(N, 0.0f);  // parte real
    std::vector<float> F2(N, 0.0f);  // parte imaginária
    std::vector<float> F(N, 0.0f);   // magnitude final (espectro)

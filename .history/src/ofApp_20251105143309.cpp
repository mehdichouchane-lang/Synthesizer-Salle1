...
void ofApp::draw(){
	...
	ofSetColor(225);
	ofDrawBitmapString("AUDIO OUTPUT EXAMPLE", 32, );
	...
	ofDrawBitmapString(reportString, 32, 379);
	// for (int i = 0; i < 12; i++) {
	// 	ofSetLineWidth(1);
	// 	if (flags[i]) {
	// 		ofSetColor(255, 0, 0);
...
float keyWidth = 70;             // largura de cada tecla
float keyHeight = 200;           // altura de cada tecla

float xOffset = 80;              // deslocamento horizontal da borda esquerda

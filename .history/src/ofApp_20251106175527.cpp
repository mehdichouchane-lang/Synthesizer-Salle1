...
void ofApp::draw(){
    ...
    for (int i = 0; i < fourierSpectrum.size()/2; i++) {
        if (fourierSpectrum[i] > maxVal) maxVal = fourierSpectrum[i];
    }

    for (int i = 0; i < fourierSpectrum.size()/2; i += 8) { // pula alguns pontos
        float x = ofMap(i, 0, fourierSpectrum.size()/2, 0, 900, true);

        // escalar para caber no retângulo de 100px
        float y = 100 - ofMap(fourierSpectrum[i], 0, maxVal, 0, 100, true);
        ofVertex(x, y);
    }
			...
			ofBeginShape();
			for (int i = 0; i < fourierSpectrum.size()/2; i += 8) { // pula alguns pontos
				float x = ofMap(i, 0, fourierSpectrum.size()/2, 0, 950, true);
				ofVertex(x, 50 -fourierSpectrum[i]*180.0f);
